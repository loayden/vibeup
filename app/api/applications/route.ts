...
function buildApplicationBackupHtml(input: ApplicationInput) {
  ...
  return `
    <body style="background:#080808;color:#ffffff;font-family:Arial,sans-serif;padding:24px;">
      ...
      <p style="margin:4px 0;color:rgba(255,255,255,0.72);"><strong>Role:</strong> ${escapeHtml(input.role)}</p>
      <p style="margin:4px 0;color:rgba(255,255,255,0.72);"><strong>Portfolio:</strong> ${escapeHtml(input.portfolioUrl || "Not provided")}</p>
      <p style="margin:4px 0;color:rgba(255,255,255,0.72);"><strong>LinkedIn:</strong> ${escapeHtml(input.linkedinUrl || "Not provided")}</p>
      <p style="margin:4px 0;color:rgba(255,255,255,0.72);"><strong>Resume:</strong> ${escapeHtml(input.resumeUrl || "Not provided")}</p>
      <div style="margin-top:18px;">
        <p style="margin:0 0 10px;color:#C6A962;"><strong>Message</strong></p>
        <p style="margin:0;color:rgba(255,255,255,0.72);white-space:pre-wrap;">${escapeHtml(message)}</p>
      </div>
    </body>
  `;
}
        ...
        sendAdminNotification(
          ...
          `
            <body style="background:#080808;color:#ffffff;font-family:Arial,sans-serif;padding:24px;">
              ...
              <pre style="white-space:pre-wrap;color:rgba(255,255,255,0.72);font-size:14px;">${escapeHtml(JSON.stringify(application, null, 2))}</pre>
            </body>
          `,
        ),
      ]);
